...
$Out = if ($env:OUT) { $env:OUT } else { "vllm-sr-all-images.tar" }
...
Write-Host "  docker images | grep -E 'vllm-sr|dashboard|envoy|vllm-sr-sim|jaeger|prometheus|grafana|redis|postgres|milvus'"
